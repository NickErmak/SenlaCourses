...
where hd >= 10*/
/*task 2.7
select p.model, price from pc 
join product p on pc.model = p.model
where p.maker like 'B%'
union
select p.model, price from printer 
join product p on printer.model = p.model
where p.maker like 'B%'
union
select p.model, price from laptop 
...
/*task 2.9
select distinct p.maker from pc, product p
...
where price > 1000*/
/*task 2.13
select avg(speed) from pc, product p
where pc.model = p.model 
and p.maker = 'SAMSUNG'*/
/*task 2.14
...
join pc as pc2 on pc1.ram = pc2.ram and pc1.speed = pc2.speed
...
/*task 2.18
select p.maker, printer.price from printer, product p
...
/*task 2.21
select maker, max(price) from pc, product p
where pc.model = p.model
group by maker*/
/*task 2.22
select speed, avg(price) from pc
...
union
select model, max(price) as price from printer) as maxPriceAll

join (select max(price) as price from 
(select model, max(price) as price from laptop
union
select model, max(price) as price from pc
union
select model, max(price) as price from printer) as T) as maxPriceTotal
on maxPriceAll.price = maxPriceTotal.price*/
/*task 2.25
